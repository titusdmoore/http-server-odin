...
parse_request :: proc(request: []u8, socket: ^net.TCP_Socket) -> (Request, bool) {
	request_parts, split_err := strings.split(
		transmute(string)request,
		"\r\n",
	);if split_err != nil {
	...
	return Request{method = method, path = path, headers = nil, body = "", client_socket = socket, raw_request = request},
		true
}
